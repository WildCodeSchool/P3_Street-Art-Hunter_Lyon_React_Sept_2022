...
export default function LeaderBoard() {
  ...
  return (
    <div>
      {isMobile && (
        <div className="text-white font-main-font h-[32vh] min-h-[15rem] overflow-auto border-y-[1px] border-white">
          <table className="w-screen text-xl backdrop-blur-sm">
            ...
            <tbody>
              {leaders.map((user, index) => (
                ...
                >
                  <td className="pl-8 flex h-[3.5rem] items-center justify-start pr-1 text-2xl w-[10w]">
                    {index + 1}
                  </td>
                  <td className="w-[45vw]">{user.pseudo}</td>
                  <td className="w-[25vw]">{user.scorepoint}</td>
                </tr>
              ))}
            </tbody>
          </table>
